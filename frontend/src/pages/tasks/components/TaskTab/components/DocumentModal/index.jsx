...
const DocumentModal = ({ open, onCancel, task, onSuccess }) => {
    ...
    const fetchDocuments = async () => {
        ...
        setLoading(true);
        try {
            const res = await getTaskDocuments({ task: task.id });
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            ...
            form.resetFields();
            setFileList([]);
            fetchDocuments(); // Siyahını yenilə
            onSuccess?.();
        } catch (error) {
    ...
    return (
        <Modal
            title={`Sənəd Əlavə Et: ${task?.title || ''}`}
            open={open}
            onCancel={onCancel}
            width={600}
            footer={null}

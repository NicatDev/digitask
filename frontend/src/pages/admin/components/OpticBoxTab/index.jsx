...
const OpticBoxTab = ({ isActive }) => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const res = await getOpticBoxes({ page_size: 100 });
    ...
    const handleDelete = async (id) => {
        try {
            await deleteOpticBox(id);
            message.success('Optik qutu silindi');
            fetchData();
        } catch (error) {
    ...
    const onFinish = async (values) => {
        try {
            if (editingItem) {
                await updateOpticBox(editingItem.id, values);
                message.success('Yeniləndi');
            } else {
                await createOpticBox(values);
    ...
    const handleStatusChange = async (id, checked) => {
        try {
            await updateOpticBox(id, { is_active: checked });
    ...
    return (
        <div>
            ...
            <Modal
                title={editingItem ? 'Optik qutunu düzəlt' : 'Yeni optik qutu'}
                open={isModalOpen}
